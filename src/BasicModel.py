...
class BasicModel(nn.Module):
    ...
    def select_interest(self, interest_eb, pos_eb):
        ...
        if self.selection == 'p':
            ...
            prob = F.softmax(torch.reshape(corr, (-1, self.interest_num))*self.tau, dim=-1) # probability of interest selection (Equation (7))
        ...
        # Bam_l: calculates the correlation score between interests and positive item by passing each of them to a linear layer
            ...
            corr = torch.matmul(comp_interest, torch.reshape(comp_pos, (-1, 16, 1))).squeeze() # inner-product of compressed multi-interest representation and embedding of positive item (Equation (9))
            prob = F.softmax(corr*self.tau, -1).squeeze()
