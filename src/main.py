...
parser.add_argument('--selection', default='hard', type=str) # the method of interest selection, 'p' for BaM_p (Equation (8)) and 'l' for BaM_l (Equation (9))
parser.add_argument('--tau', default=1, type=float) # the softness of selection (refer to Equation (7))
...
if __name__ == '__main__':
    ...
    if args.model == 'comirec':
        model = ComiRec(item_count, args.hidden_size, args.batch_size, args.selection, args.tau, args.interest_num, seq_len, add_pos=args.add_pos, linear_size = args.linear_size).to(args.device)
    elif args.model == 'mind':
        model = MIND(item_count, args.hidden_size, args.batch_size, args.interest_num, seq_len, routing_times=3, relu_layer=False, selection=args.selection, tau=args.tau, linear_size = args.linear_size).to(args.device)
