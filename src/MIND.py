...
class MIND(BasicModel):
    ...
    # initialization of model
    def __init__(self, item_num, hidden_size, batch_size, interest_num=4, seq_len=50, routing_times=3, relu_layer=True, selection='hard', tau=1, linear_size = 16):
        ...
        self.tau = tau
        if self.selection == 'l':
            self.l1 = nn.Linear(self.hidden_size, linear_size, bias=False)
            self.l2 = nn.Linear(self.hidden_size, linear_size, bias=False)
