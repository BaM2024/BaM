...
def data_partition(dataset):
    ...
    # f = open('data/%s_tail' % dataset, 'r')
    # tails = [int(i) for i in f.read().split()]
...
def get_distance(emb, k, device, epsilon=1e-8,):
    emb = emb.squeeze().t()
    diffs = emb - emb.unsqueeze(1)
    distances = torch.norm(diffs, dim=2)
    average_distance = distances.sum() / (distances.numel() - distances.size(0))
    # norm_embeddings = F.normalize(emb, p=2, dim=-1, eps=epsilon)
    # cos_sim = torch.matmul(norm_embeddings, norm_embeddings.transpose(-1, -2))    
    # eye = torch.eye(k, device=device).unsqueeze(0)
    # cos_sim = cos_sim * (1 - eye)
    # cos_sim = (cos_sim/2).mean()
